...
for (let row = 0; row < 500; row++) {
  ...
  sheetStore.push(rowData);
}
function App() {
  // const [sheetDB, setSheetDB] = useState(() => [...sheetStore]);

  // Efficient way to update the value of a specific cell
  ...
  function updateCell(cellProperties) {
    setSheetData((prev) => {
      return prev.map((r, i) => {
        return r.map((c, j) => {
          const str = c.id.split("-");
          const rowIndex = Number(str[0]); // Convert to number
          const colIndex = Number(str[1]); // Convert to number

          // Check if the current cell matches the coordinates
          // if (i === rowIndex && j === colIndex) {
          //   return {
          //     ...cellProperties, // Update cell with new properties
          //   };
          // }
          if (c.id === cellProperties.id) {
            return {
              ...c,
              ...cellProperties, // Update cell with new properties
            };
  ...
  const handleInputChange = (e) => {
    // setSharedValue(e.target.innerText);
    console.log("typing");
    const newValue = e.target.innerText;
    setSharedValue(newValue);
    moveCaretToEnd();
    // Update the active cell's value in the sheetDB
  ...
  const moveCaretToEnd = () => {
    ...
    range.collapse(false); // Collapse the range to the end (false means to the end of the range)
    selection.removeAllRanges(); // Clear any existing selection
    selection.addRange(range); // Add the range at the end
  };
